...
inline struct RadixTreeNodePtr radix_tree_create_leaf_node(
    ...
    const uint32_t idx =
        freelist_insert_unsafe(&(tree)->leaves, value_ptr, value_size);
    return (struct RadixTreeNodePtr){.type = RTT_LEAF, .idx = idx};
}

inline struct RadixTreeNodePtr radix_tree_create_branch_node(
    _RadixTreeBase* tree,
    const enum RadixTreeNodeType branch_type
) {
    switch (branch_type) {
    case RTT_BRANCH_4: {
        const uint32_t idx = freelist_insert(
            &tree->branches4,
            (const struct RadixTreeBranch4Node){0}
        );
    ...
    case RTT_BRANCH_8: {
        const uint32_t idx = freelist_insert(
            &tree->branches8,
            (const struct RadixTreeBranch8Node){0}
        );
        return (struct RadixTreeNodePtr){.type = branch_type, .idx = idx};
    }
    case RTT_BRANCH_16: {
        const uint32_t idx = freelist_insert(
            &tree->branches16,
            (const struct RadixTreeBranch16Node){0}
        );
        return (struct RadixTreeNodePtr){.type = branch_type, .idx = idx};
    }
    case RTT_BRANCH_FULL: {
        const uint32_t idx = freelist_insert(
            &tree->branches_full,
            (const struct RadixTreeBranchFullNode){0}
        );
        return (struct RadixTreeNodePtr){.type = branch_type, .idx = idx};
    }
    default:
...
inline struct RadixTreeNodePtr radix_tree_create_edge_node(
    _RadixTreeBase* tree,
    const radix_key_t* key,
    const radix_key_idx_t key_size,
    const radix_key_idx_t offset,
    const radix_key_idx_t key_length,
    const struct RadixTreeNodePtr next
) {
    ...
    if (edge.length > RADIX_TREE_SMALL_STR_SIZE) {
        ...
        tree->edges.data[idx].string_ptr =
            malloc((edge.length + 1) / RADIX_TREE_KEY_RATIO);
        memcpy(
            tree->edges.data[idx].string_ptr,
            buf,
            (edge.length + 1) / RADIX_TREE_KEY_RATIO
        );
#else
        tree->edges.data[idx].string_idx =
            freelist_insert_unsafe(&tree->strings, buf, key_size);
#endif
    } else {
        memcpy(
            tree->edges.data[idx].data,
            buf,
            (edge.length + 1) / RADIX_TREE_KEY_RATIO
        );
    }
...
inline radix_key_t radix_tree_key_unpack(
    const radix_key_t* key,
    const radix_key_idx_t idx
) {
    return (key[idx >> 1] >> ((~idx & 1) << 2)) & 0xf;
}

inline void radix_tree_key_pack(
    radix_key_t* key,
    const radix_key_idx_t idx,
    const radix_key_t value
) {
    key[idx >> 1] = (key[idx >> 1] & (0xf << ((idx & 1) << 2))) |
                    (value << ((~idx & 1) << 2));
}

inline void radix_tree_copy_key(
    radix_key_t* dest,
    const radix_key_t* src,
    const radix_key_idx_t offset,
    const radix_key_idx_t length
) {
    if ((offset & 1) == 0) {
        memcpy(
            dest,
            src + (offset / RADIX_TREE_KEY_RATIO),
            (length + 1) / RADIX_TREE_KEY_RATIO
        );
        return;
    }
...
inline void radix_tree_get_branch(
    const _RadixTreeBase* tree,
    const struct RadixTreeNodePtr* node,
    const radix_key_t key,
    struct RadixTreeNodePtr* out
) {
    switch (node->type) {
    case RTT_BRANCH_4: {
    ...
    case RTT_BRANCH_16: {
        radix_tree_get_branch16(
            radix_tree_fetch_branch16(tree, *node),
            key,
            out
        );
    } break;
    case RTT_BRANCH_FULL: {
        radix_tree_get_branch_full(
            radix_tree_fetch_branch_full(tree, *node),
            key,
            out
        );
    } break;
...
inline void radix_tree_get_branch4(
    const struct RadixTreeBranch4Node* branch,
    const radix_key_t key,
...
inline void radix_tree_get_branch8(
    const struct RadixTreeBranch8Node* branch,
    const radix_key_t key,
    struct RadixTreeNodePtr* node
) {
...
inline void radix_tree_get_branch16(
    const struct RadixTreeBranch16Node* branch,
    const radix_key_t key,
    struct RadixTreeNodePtr* node
) {
...
inline void radix_tree_get_branch_full(
    const struct RadixTreeBranchFullNode* branch,
    const radix_key_t key,
    struct RadixTreeNodePtr* node
) {
    *node = branch->entries[key];
...
inline void radix_tree_set_prev(
    _RadixTreeBase* tree,
    const struct RadixTreeNodePtr new_node
) {
    ...
    for (;;) {
        ...
        case RTT_EDGE: {
            struct RadixTreeEdgeNode* edge =
                radix_tree_fetch_edge(tree, prev->node);
            edge->next = node;
            return;
        } break;
        case RTT_BRANCH_4:
        ...
        case RTT_BRANCH_16:
        case RTT_BRANCH_FULL: {
            node = radix_tree_insert_into_branch_or_grow(
                tree,
                prev->node,
                prev->branch_val,
                node
...
inline struct RadixTreeNodePtr radix_tree_insert_into_branch_or_grow(
    _RadixTreeBase* tree,
    const struct RadixTreeNodePtr branch_node,
    const radix_key_t key,
    const struct RadixTreeNodePtr node
) {
    switch (branch_node.type) {
    case RTT_BRANCH_4: {
        struct RadixTreeBranch4Node* branch =
            radix_tree_fetch_branch4(tree, branch_node);
        const struct RadixTreeNodePtr new_node =
            radix_tree_insert_into_branch4_or_grow(tree, branch, key, node);
    ...
    case RTT_BRANCH_8: {
        struct RadixTreeBranch8Node* branch =
            radix_tree_fetch_branch8(tree, branch_node);
        const struct RadixTreeNodePtr new_node =
            radix_tree_insert_into_branch8_or_grow(tree, branch, key, node);
    ...
    case RTT_BRANCH_16: {
        struct RadixTreeBranch16Node* branch =
            radix_tree_fetch_branch16(tree, branch_node);
        const struct RadixTreeNodePtr new_node =
            radix_tree_insert_into_branch16_or_grow(tree, branch, key, node);
    ...
    case RTT_BRANCH_FULL: {
        struct RadixTreeBranchFullNode* branch =
            radix_tree_fetch_branch_full(tree, branch_node);
...
inline struct RadixTreeNodePtr radix_tree_insert_into_branch16_or_grow(
    ...
    if (key == RADIX_BRANCH_IMMEDIATE) {
        ...
        memcpy(
            &new_branch.entries,
            branch->entries,
            sizeof(struct RadixTreeBranch16Node)
        );
...
inline void radix_tree_push_prev(
    _RadixTreeBase* tree,
    const struct RadixTreeNodePtr node,
    const radix_key_idx_t branch_val
) {
    if (node.type == RTT_EDGE) {
        tree->prev.len = 0;
    }

    const struct RadixTreePrevPtr new_prev = {
        .node = node,
...
void _radix_tree_insert(
    _RadixTreeBase* tree,
    const radix_key_t* new_key,
    const radix_key_idx_t key_len,
    const radix_key_idx_t key_size,
    const void* value,
    const uint32_t value_size
) {
    ...
    tree->prev.len = 0;
    radix_tree_push_prev(
        tree,
        (struct RadixTreeNodePtr){.type = RTT_NONE},
        RADIX_BRANCH_IMMEDIATE
    );
    ...
    for (;;) {
        ...
        case RTT_EDGE: {
            ...
            const radix_key_t* old_key =
                radix_tree_fetch_edge_str_unsafe(tree, edge, key_size);
            ...
            if (old_val != new_val) {
                ...
                struct RadixTreeNodePtr new_node =
                    radix_tree_create_leaf_node(tree, value, value_size);
                ...
                radix_tree_refetch_edge_str_unsafe(
                    old_key,
                    tree,
                    edge,
                    key_size
                ...
                const struct RadixTreeNodePtr branch_node =
                    radix_tree_create_branch_node(tree, RTT_BRANCH_4);
                struct RadixTreeBranch4Node* branch =
                    radix_tree_fetch_branch4(tree, branch_node);
            ...
            // old key has remainder + new key has remainder -> new branch index
            // on val 1 and 2 one key has remainder + other does not -> branch
            // with immediate value both keys are exhausted -> leaf node (do
            // nothing as we do not replace values)
            ...
            if (old_has_remainder && new_has_remainder) {
                ...
                struct RadixTreeBranch4Node* branch =
                    radix_tree_fetch_branch4(tree, edge->next);
                branch->values[0] = old_val;
                branch->entries[0] = old_node;
                branch->values[1] = new_val;
                branch->entries[1] = new_node;
            ...
            if (old_has_remainder && !new_has_remainder) {
                const struct RadixTreeNodePtr new_branch_node =
                    radix_tree_create_branch_node(tree, RTT_BRANCH_4);
                struct RadixTreeBranch4Node* new_branch =
                    radix_tree_fetch_branch4(tree, new_branch_node);
                struct RadixTreeNodePtr new_node =
                    radix_tree_create_leaf_node(tree, value, value_size);
                struct RadixTreeNodePtr old_node = edge->next;
                ...
                radix_key_t branch_val =
                    radix_tree_key_unpack(old_key, old_idx);
            ...
            // consumed entire old key, but we need to split the edge at the
            // last value and let the old one immediately point to the new leaf.
            if (!old_has_remainder && new_has_remainder &&
                edge->next.type == RTT_LEAF) {
                const struct RadixTreeNodePtr new_branch_node =
                    radix_tree_create_branch_node(tree, RTT_BRANCH_4);
                struct RadixTreeBranch4Node* new_branch =
                    radix_tree_fetch_branch4(tree, new_branch_node);
            ...
            // If we have matched every single character in the edge key, the
            // next node is not a leaf and our own key is not exhausted we need
        ...
        case RTT_BRANCH_FULL: {
            ...
            const radix_key_t branch_val =
                radix_tree_key_unpack(new_key, key_idx++);
            ...
            case RTT_EDGE: {
                ...
                // Special case 1: We have consumed the entire key but the next
                // key is an edge we need to replace it with a branch and set
                // our value as its immediate value while adding the edge to the
                // branch by consuming one character from it.

                // replace edge with branch that takes next edge key value and
                // this leaf as an immediate value
                struct RadixTreeEdgeNode* edge =
                    radix_tree_fetch_edge(tree, next);
                const radix_key_t* old_key =
                    radix_tree_fetch_edge_str_unsafe(tree, edge, key_size);
                // first next edge key value
                const radix_key_t old_val = radix_tree_key_unpack(old_key, 0);

                struct RadixTreeNodePtr new_branch_node =
                    radix_tree_create_branch_node(tree, RTT_BRANCH_4);
                ...
                new_branch->values[0] = RADIX_BRANCH_IMMEDIATE;
                new_branch->entries[0] =
                    radix_tree_create_leaf_node(tree, value, value_size);

                // simple case where we can just insert the edge next as the
                // node for the edge.
                if (edge->length == 1) {
                    ...
                    const struct RadixTreeNodePtr maybe_new =
                        radix_tree_insert_into_branch_or_grow(
                            tree,
                            node,
                            branch_val,
                            new_branch_node
                ...
                // we have to extract the first character of the edge key and
                // create a new edge
                struct RadixTreeNodePtr new_edge_node =
                    radix_tree_create_edge_node(
                        tree,
                        old_key,
                        key_size,
                        1,
                ...
                // Since we have to delete the old edge we need to clean up if
            ...
            case RTT_BRANCH_FULL: {
                ...
                // Special case 2: We have consumed the entire key but the next
                // node is a branch we need to fill in the immediate field with
                // our value.
            ...
            case RTT_LEAF: {
                ...
                // Special case 3: The next node from the branch is a leaf but
                // our current key has a remainder we have to branch and leave
                // the old leaf as the immediate value while continuing with the
                // new key.
        ...
        case RTT_LEAF: {
            ...
            memcpy(
                freelist_get_unsafe(&tree->leaves, node.idx, value_size),
                value,
                value_size
            );
            return;
...
void _radix_tree_get(
    ...
    for (;;) {
        switch (node.type) {
        case RTT_EDGE: {
            const struct RadixTreeEdgeNode* edge =
                radix_tree_fetch_edge(tree, node);
            const radix_key_t* edge_key =
                radix_tree_fetch_edge_str_unsafe(tree, edge, key_size);
            ...
            for (; old_idx < edge->length; old_idx++) {
                if (radix_tree_key_unpack(edge_key, old_idx) !=
                    radix_tree_key_unpack(key, key_idx++)) {
                    goto notfound;
                }
            }
        ...
        case RTT_BRANCH_4: {
            const struct RadixTreeBranch4Node* branch =
                radix_tree_fetch_branch4(tree, node);

            const radix_key_t val = key_len <= key_idx
                                        ? RADIX_BRANCH_IMMEDIATE
                                        : radix_tree_key_unpack(key, key_idx++);
        ...
        case RTT_BRANCH_8: {
            const struct RadixTreeBranch8Node* branch =
                radix_tree_fetch_branch8(tree, node);
            const radix_key_t val = key_len <= key_idx
                                        ? RADIX_BRANCH_IMMEDIATE
                                        : radix_tree_key_unpack(key, key_idx++);
