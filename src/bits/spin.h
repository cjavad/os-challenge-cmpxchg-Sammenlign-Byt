...
inline void spin_rwlock_init(
    struct SRWLock* lock
) {
    atomic_store(&lock->lock, 0);
}

inline void spin_rwlock_rdlock(
    struct SRWLock* lock
) {
    while (true) {
...
inline void spin_rwlock_rdunlock(
    struct SRWLock* lock
) {
    atomic_fetch_sub(&lock->lock, 1);
}

inline void spin_rwlock_wrlock(
    struct SRWLock* lock
) {
    uint32_t e = 0;
...
inline void spin_rwlock_wrunlock(
    struct SRWLock* lock
) {
