...
inline uint32_t priority_heap_parent(const uint32_t i) { return (i - 1) / 2; }
inline uint32_t priority_heap_left(const uint32_t i) { return 2 * i + 1; }
inline uint32_t priority_heap_right(const uint32_t i) { return 2 * i + 2; }

void priority_heap_shift_up(const _AnyPriorityHeap* heap, uint32_t i, const uint32_t node_size) {
    while (i > 0) {
        const uint32_t parent = priority_heap_parent(i);

        const _AnyPriorityHeapNode* current = vec_get_unsafe(heap, i, node_size);
...
void _priority_heap_get_max(const _AnyPriorityHeap* heap, const uint32_t node_size, _AnyPriorityHeapNode** node) {
    if (heap->len == 0) {
        *node = NULL;
        return;
    }
...
void _priority_heap_extract_max(_AnyPriorityHeap* heap, const uint32_t node_size, _AnyPriorityHeapNode* node) {
    if (heap->len == 0) {
        return;
    }
