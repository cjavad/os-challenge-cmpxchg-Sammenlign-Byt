...
// Allow us to change the radix_key_idx_t size to gain more string space in the
// edge node.
#if RADIX_TREE_MALLOC_STRS
#define RADIX_TREE_EDGE_PADDING                                                \
    (12 - sizeof(radix_key_t*) - sizeof(radix_key_idx_t))
#define RADIX_TREE_EDGE_DATA_SIZE                                              \
    (sizeof(radix_key_t*) + RADIX_TREE_EDGE_PADDING)
#else
#define RADIX_TREE_EDGE_PADDING   (4 - sizeof(radix_key_idx_t))
#define RADIX_TREE_EDGE_DATA_SIZE (4 + RADIX_TREE_EDGE_PADDING)
#endif

#define RADIX_TREE_SMALL_STR_SIZE                                              \
    (RADIX_TREE_EDGE_DATA_SIZE * RADIX_TREE_KEY_RATIO)
...
struct RadixTreeNodePtr radix_tree_create_branch_node(
    _RadixTreeBase* tree,
    enum RadixTreeNodeType branch_type
);

struct RadixTreeNodePtr radix_tree_create_leaf_node(
    _RadixTreeBase* tree,
...
struct RadixTreeNodePtr radix_tree_create_edge_node(
    _RadixTreeBase* tree,
    const radix_key_t* key,
    radix_key_idx_t key_size,
    radix_key_idx_t offset,
    radix_key_idx_t key_length,
    struct RadixTreeNodePtr next
);
...
void radix_tree_get_branch4(
    const struct RadixTreeBranch4Node* branch,
    radix_key_t key,
    struct RadixTreeNodePtr* node
);
void radix_tree_get_branch8(
...
struct RadixTreeNodePtr radix_tree_insert_into_branch_or_grow(
    _RadixTreeBase* tree,
    struct RadixTreeNodePtr branch_node,
    radix_key_t key,
    struct RadixTreeNodePtr node
);
...
void radix_tree_push_prev(
    _RadixTreeBase* tree,
    struct RadixTreeNodePtr node,
    radix_key_idx_t branch_val
);

void _radix_tree_insert(
    _RadixTreeBase* tree,
    const radix_key_t* new_key,
    radix_key_idx_t key_len,
    radix_key_idx_t key_size,
    const void* value,
    uint32_t value_size
);
...
#define radix_tree_fetch_leaf_unsafe(tree, ptr, size)                          \
    freelist_get_unsafe(&(tree)->leaves, (ptr).idx, (size))
...
#define radix_tree_fetch_branch16(tree, ptr)                                   \
    (&(tree)->branches16.data[(ptr).idx])
#define radix_tree_fetch_branch_full(tree, ptr)                                \
    (&(tree)->branches_full.data[(ptr).idx])
...
#define radix_tree_fetch_edge_str_unsafe(tree, edge, key_size)                 \
    ((edge)->length > RADIX_TREE_SMALL_STR_SIZE ? (edge)->string_ptr           \
                                                : (radix_key_t*)(edge)->data)
#define radix_tree_refetch_edge_str_unsafe

#else
#define radix_tree_fetch_edge_str_unsafe(tree, edge, key_size)                 \
    ((edge)->length > RADIX_TREE_SMALL_STR_SIZE ? (freelist_get_unsafe(        \
                                                      &(tree)->strings,        \
                                                      (edge)->string_idx,      \
                                                      (key_size)               \
                                                  ))                           \
...
#define radix_tree_create(tree, default_cap)                                   \
    {                                                                          \
        ...
        freelist_init_unsafe(                                                  \
            &(tree)->strings,                                                  \
            radix_tree_key_length(tree) > RADIX_TREE_SMALL_STR_SIZE            \
                ? default_cap                                                  \
                : 1,                                                           \
...
#define ____RT_TEMP(x, c)                                                      \
    ____CONCAT(____CONCAT(____CONCAT(____rt_temp, x), ____), c)

#if RADIX_TREE_MALLOC_STRS
#define ____radix_tree_edge_maybe_move_str(tree, edge, old_key, old_idx, c)    \
    {                                                                          \
        if ((edge)->length > RADIX_TREE_SMALL_STR_SIZE &&                      \
            (old_idx) <= RADIX_TREE_SMALL_STR_SIZE) {                          \
            radix_key_t* ____RT_TEMP(old_str_ptr, c) = (edge)->string_ptr;     \
            memcpy(                                                            \
                (edge)->data,                                                  \
                ____RT_TEMP(old_str_ptr, c),                                   \
                (old_idx) + 1 / RADIX_TREE_KEY_RATIO                           \
            );                                                                 \
...
#define radix_tree_edge_maybe_move_str(tree, edge, old_key, old_idx)           \
    ____radix_tree_edge_maybe_move_str(                                        \
        tree,                                                                  \
        edge,                                                                  \
        old_key,                                                               \
        old_idx,                                                               \
