...
// SHA256
int sha256_hash_openssl(const uint8_t *data, const size_t len, hash_u *hash) {
    EVP_Q_digest(NULL, "SHA256", NULL, data, len, hash->u8, NULL);
    return 0;
}
...
uint64_t reverse_hash(uint64_t start, uint64_t end, hash_u target) {
    ...
    for (uint64_t i = start; i < end; i++) {
        ...
        sha256_hash_64(out, (uint8_t*)&i);

        if (memcmp(out, target.u8, 32) == 0) {
            return i;
        }
    }
