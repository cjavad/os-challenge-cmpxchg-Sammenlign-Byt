...
inline void protocol_request_to_le(ProtocolRequest* req) {
    req->start = __builtin_bswap64(req->start);
    req->end = __builtin_bswap64(req->end);
}

inline void protocol_response_to_be(ProtocolResponse* resp) { resp->answer = __builtin_bswap64(resp->answer); }
