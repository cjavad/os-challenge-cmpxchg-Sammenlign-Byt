...
scheduler_priority_create(const uint32_t default_cap) {
  struct PriorityScheduler *scheduler =
      calloc(1, sizeof(struct PriorityScheduler));
...
SchedulerJobId
scheduler_priority_submit(struct PriorityScheduler *scheduler,
  ...
  radix_tree_get(&scheduler->base.cache->tree, request->hash,
                 SHA256_DIGEST_LENGTH, &cached_answer);
  ...
  for (uint32_t i = 0; i < scheduler->jobs_w->len; i++) {
    ...
    const struct PrioritySchedulerJob *some_job =
        &scheduler->jobs.data[some_job_idx];

    if (scheduler_priority_job_is_done(some_job) &&
        atomic_load(&some_job->rc) == 0) {
...
void scheduler_priority_cancel(const struct PriorityScheduler *scheduler,
                               const SchedulerJobId job_id) {
  for (uint32_t i = 0; i < scheduler->jobs_r->len; i++) {
    struct PrioritySchedulerJob *job =
        &scheduler->jobs.data[scheduler->jobs_r->data[i].elem];
...
bool scheduler_priority_schedule(struct PriorityScheduler *scheduler,
                                 IndexPriorityHeap *local_jobs,
                                 SchedulerJobId *prev_max_job_id,
                                 SchedulerJobId *prev_job_id,
                                 HashDigest target_hash, uint32_t *job_idx,
                                 uint64_t *start, uint64_t *end) {

  // First attempt to get the newest version of the jobs heap (if the job id
  // is newer)
...
void *scheduler_priority_worker(struct PriorityScheduler *scheduler) {
  ...
  while (scheduler->base.running) {
    ...
    if (!scheduler_priority_schedule(scheduler, &local_jobs, &prev_max_job_id,
                                     &prev_job_id, target_hash, &job_idx,
                                     &start, &end)) {
      continue;
    }
    ...
    // Answer was found, extract recipient ptr and mark job as done (it will
    // be deleted).
    spin_rwlock_rdlock(&scheduler->rlock);
    struct PrioritySchedulerJob *job = &scheduler->jobs.data[job_idx];
