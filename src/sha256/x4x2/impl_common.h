...
#define M128I_FUSE_PS(l, u) _mm256_castsi256_ps(_mm256_insertf128_si256(_mm256_castsi128_si256(l), u, 1))
...
static __m128i m128_rotr(const __m128i x, const uint32_t n) {
    return _mm_or_si128(_mm_srli_epi32(x, n), _mm_slli_epi32(x, 32 - n));
}

static __m128i m128_s0(const __m128i w) {
...
static __m128i m128_s1(const __m128i w) {
...
static __m128i m128_S0(const __m128i a) {
    return _mm_castps_si128(
        _mm_xor_ps(_mm_castsi128_ps(m128_rotr(a, 2)),
                   _mm_xor_ps(_mm_castsi128_ps(m128_rotr(a, 13)),
                              _mm_castsi128_ps(m128_rotr(a, 22)))));
}

static __m128i m128_S1(const __m128i e) {
    return _mm_castps_si128(
        _mm_xor_ps(_mm_castsi128_ps(m128_rotr(e, 6)),
                   _mm_xor_ps(_mm_castsi128_ps(m128_rotr(e, 11)),
                              _mm_castsi128_ps(m128_rotr(e, 25)))));
}

static __m256 m256_ch(const __m256 e, const __m256 f, const __m256 g) {
	return _mm256_xor_ps(
		_mm256_and_ps(e, f),
		_mm256_andnot_ps(e, g)
	);
}

static __m256 m256_maj(const __m256 a, const __m256 b, const __m256 c) {
	return _mm256_xor_ps(
		_mm256_and_ps(a, b),
		_mm256_xor_ps(
			_mm256_and_ps(a, c), 
