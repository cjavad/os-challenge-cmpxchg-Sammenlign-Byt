...
__attribute__((flatten)) void sha256x4x2_optim(
...

    {
        ...
        __m128i mask0 = _mm_load_si128((__m128i*)byteswap_load0),
                mask1 = _mm_load_si128((__m128i*)&byteswap_load1),
        ...
        __m128i hl0 = _mm_shuffle_epi8(ll0, mask0),
                hl1 = _mm_shuffle_epi8(ll1, mask0),
                hu0 = _mm_shuffle_epi8(lu0, mask1),
                hu1 = _mm_shuffle_epi8(lu1, mask1);

        __m256 h0 = M128I_FUSE_PS(hl0, hl1), h1 = M128I_FUSE_PS(hu0, hu1);

        __m256 w0 = _mm256_blend_ps(h0, h1, 0b11001100),
               w1 = _mm256_blend_ps(h0, h1, 0b00110011);
    ...
    {
        __m128i w17l =
            _mm_add_epi32(t1l, _mm_add_epi32(m128_s0(c80), m128_s1(c64)));
        __m128i w17u =
            _mm_add_epi32(t1u, _mm_add_epi32(m128_s0(c80), m128_s1(c64)));
    ...
    }
    {
        ...
        ll = M256_LOWER_SI(l1);
        __m128i w30l =
            _mm_add_epi32(m128_s0(c64), _mm_add_epi32(ll, m128_s1(t0l)));
        __m128i w30u =
            _mm_add_epi32(m128_s0(c64), _mm_add_epi32(lu, m128_s1(t0u)));
        ...
        lu = M256_UPPER_SI(l0);
        ll = M256_LOWER_SI(l0);
        __m128i w31l = _mm_add_epi32(
            _mm_add_epi32(ll, m128_s1(t1l)),
            _mm_add_epi32(c64, m128_s0(M256_LOWER_SI(l1)))
        );
        __m128i w31u = _mm_add_epi32(
            _mm_add_epi32(lu, m128_s1(t1u)),
            _mm_add_epi32(c64, m128_s0(M256_UPPER_SI(l1)))
        );
    ...
    for (uint32_t i = 32; i < 64; i++) {
        __m128i wi16l = _mm_load_si128(&w[i * 2 - 32]),
                wi16u = _mm_load_si128(&w[i * 2 - 31]),
                wi15l = _mm_load_si128(&w[i * 2 - 30]),
                wi15u = _mm_load_si128(&w[i * 2 - 29]),
                wi7l = _mm_load_si128(&w[i * 2 - 14]),
                wi7u = _mm_load_si128(&w[i * 2 - 13]), wi2l = t0l, wi2u = t0u;

        t0l = t1l;
        t0u = t1u;

        __m128i ws1l = m128_s1(wi2l), ws1u = m128_s1(wi2u),
                ws0l = m128_s0(wi15l), ws0u = m128_s0(wi15u);

        __m128i wil = _mm_add_epi32(
                    _mm_add_epi32(wi16l, wi7l),
                    _mm_add_epi32(ws0l, ws1l)
                ),
                wiu = _mm_add_epi32(
                    _mm_add_epi32(wi16u, wi7u),
                    _mm_add_epi32(ws0u, ws1u)
                );
    ...
    __m256 a = _mm256_permute_ps(h0, 0b00000000),
           b = _mm256_permute_ps(h0, 0b01010101),
           c = _mm256_permute_ps(h0, 0b10101010),
           d = _mm256_permute_ps(h0, 0b11111111),
           e = _mm256_permute_ps(h1, 0b00000000),
           f = _mm256_permute_ps(h1, 0b01010101),
           g = _mm256_permute_ps(h1, 0b10101010),
           h = _mm256_permute_ps(h1, 0b11111111);

    for (uint32_t i = 0; i < 64; i++) {
        __m128i ki = _mm_castps_si128(_mm_broadcast_ss((float*)&k[i])),
                wil = _mm_load_si128(&w[i * 2]),
        ...
        __m128i
            temp2u =
                _mm_add_epi32(m128_S0(M256_UPPER_SI(a)), M256_UPPER_SI(maj)),
            temp2l =
                _mm_add_epi32(m128_S0(M256_LOWER_SI(a)), M256_LOWER_SI(maj)),
        ...
        f = e;
        e = M128I_FUSE_PS(
            _mm_add_epi32(M256_LOWER_SI(d), temp1l),
            _mm_add_epi32(M256_UPPER_SI(d), temp1u)
        );
        d = c;
        c = b;
        b = a;
        a = M128I_FUSE_PS(
            _mm_add_epi32(temp1l, temp2l),
            _mm_add_epi32(temp1u, temp2u)
        );
    }
    ...
    // rows without swap
    __m256i
        r0 = _mm256_castps_si256(_mm256_shuffle_ps(tmp00, tmp02, 0b01000100)),
