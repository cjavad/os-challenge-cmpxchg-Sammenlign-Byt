...
#define ____freelist_maybe_grow(freelist, size, iii)                                                                   \
    {                                                                                                                  \
        if ((freelist)->free == 0) {                                                                                   \
            ...
            (freelist)->data = reallocarray((freelist)->data, (freelist)->cap, size);                                  \
            (freelist)->indicices =                                                                                    \
...
#define freelist_insert(freelist, elem)                                                                                \
    ({                                                                                                                 \
        ____freelist_maybe_grow(freelist, (sizeof(*(freelist)->data)), __COUNTER__);                                   \
...
#define freelist_insert_unsafe(freelist, ptr, size)                                                                    \
    ({                                                                                                                 \
        ...
        memcpy(((void*)(freelist)->data) + (((freelist)->indicices[--(freelist)->free]) * size), ptr, size);           \
        (freelist)->indicices[(freelist)->free];                                                                       \
    })

#define freelist_get_unsafe(freelist, idx, size)                                                                       \
    ({ (typeof((freelist)->data))(((void*)(freelist)->data) + ((idx) * (size))); })
